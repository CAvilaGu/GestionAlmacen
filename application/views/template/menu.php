        <nav class="navbar-default navbar-side gray-light" role="navigation">
            <div class="sidebar-collapse" style="background-color: #DCDCDC; border-right: black 2px solid;">
                <ul class="nav" id="main-menu">
                    <li class="text-center"><h4><strong>Compras</strong></h4></li>
                    <li>
                        <a href="index.html"><i class="fa fa-cubes fa-3x text-justify"></i> Entrada a Almacén</a>
                    </li>
                    <li>
                        <a  href="ui.html"><i class="fa fa-industry fa-3x text-justify"></i> Proveedores</a>
                    </li>
                    <li class="text-center"><h4><strong>Ventas</strong></h4></li>
                    <li>
                        <a  href="tab-panel.html"><i class="fa fa-qrcode fa-3x text-justify"></i> Despacho</a>
                    </li>
                    <li  >
                        <a   href="chart.html"><i class="fa fa-bar-chart-o fa-3x text-justify"></i> Producción</a>
                    </li> 
                    <li  >
                        <a  href="table.html"><i class="fa fa-users fa-3x text-justify"></i> Clientes</a>
                    </li>
                    <li class="text-center"><h4><strong>Almacén</strong></h4></li>
                    <li  >
                        <a  href="form.html"><i class="fa fa-edit fa-3x text-justify"></i> Productos </a>
                    </li>       
                    <li  >
                        <a   href="login.html"><i class="fa fa-bolt fa-3x text-justify"></i> Inventario</a>
                    </li> 
                    <li  >
                        <a   href="registeration.html"><i class="fa fa-laptop fa-3x text-justify"></i> Almacenes</a>
                    </li> 
                </ul>

            </div>
        </nav>  